...
const CheckoutPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (missing.length > 0) {
        enqueueSnackbar(`Please fill in all fields: ${missing.join(", ")}`, {
          variant: "warning",
        });
        setLoading(false);
        return;
      ...
      if (selectedAddressId === "new" && saveAddress) {
        try {
          ...
          enqueueSnackbar("Address saved for future use", {
            variant: "success",
          });
        } catch (saveErr) {
          console.error("Failed to save address", saveErr);
          enqueueSnackbar("Failed to save address, proceeding with order...", {
            variant: "warning",
          });
        }
      }
...
<>
    ...
    <div className="container mx-auto  ">
      <div className="relative bg-slate-100 py-3 overflow-hidden marquee-wrapper">
        {/* Scrolling Content */}
        <div className="flex w-max marquee-content gap-11">
          <span className="whitespace-nowrap text-gray-800 font-semibold px-12 mr-50">
            🚚 We are currently offering Cash on Delivery for your convenience •
          </span>
          <span className="whitespace-nowrap text-gray-800 font-semibold px-12">
            🚚 We are currently offering Cash on Delivery for your convenience •
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm sticky top-24">
            ...
            <div className="flex flex-wrap gap-4 p-4 justify-center sm:justify-around w-full">
              {/* Cash on Delivery Option */}
              <div
                className={`flex flex-col items-center justify-center w-40 p-4 border-2 rounded-2xl cursor-pointer transition-all duration-200 ease-in-out
  ${
    paymentMethod === "cod"
      ...
      : "border-gray-100 hover:border-blue-500 hover:shadow-md"
              ...
              >
                <div className="p-3 mb-3 text-emerald-600 bg-emerald-50 rounded-full group-hover:scale-110 transition-transform duration-200">
                  {/* Cash / Banknote Icon */}
                  <svg
     ...
     ${
       paymentMethod === "upi"
         ...
         : "border-gray-100 hover:border-blue-500 hover:shadow-md"
                ...
                onClick={() => handlePayUsingUpi()}
              >
                <div className="p-3 mb-3 text-indigo-600 bg-indigo-50 rounded-full group-hover:scale-110 transition-transform duration-200">
                  {/* Smartphone / Scan / UPI Icon */}
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="28"
                    height="28"
            ...
            <button
              ...
              disabled={
                loading ||
                !paymentMethod ||
                (selectedAddressId === "new" && !formData.fullName)
              }
